...
(async () => {
 ...
 global.fetch = async () => { tileFetched = true; throw new Error('direct tile URL must not fetch'); };
 const directTile = await parseWMTS('https://tile.focusit.tw/wmts/dark_osm/18/219403/112289.png');
 assert.equal(tileFetched,false);
 assert.equal(directTile[0].mapData.kind,'xyz');
 assert.equal(directTile[0].mapData.url,'https://tile.focusit.tw/wmts/dark_osm/{z}/{x}/{y}.png');
 const dollarTemplate = await parseWMTS('https://tile.focusit.tw/wmts/dark_osm/${z}/${x}/${y}.png');
 assert.equal(tileFetched,false);
 assert.equal(dollarTemplate[0].mapData.url,'https://tile.focusit.tw/wmts/dark_osm/{z}/{x}/{y}.png');
 const xyzTemplate = await parseWMTS('https://tile.focusit.tw/wmts/dark_osm/{z}/{x}/{y}.png');
 assert.equal(tileFetched,false);
 assert.equal(xyzTemplate[0].mapData.url,'https://tile.focusit.tw/wmts/dark_osm/{z}/{x}/{y}.png');
 global.includeMap = false;
